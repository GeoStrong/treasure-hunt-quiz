...
const EgyptQuizzQuestion2: React.FC = () => {
  ...
  useEffect(() => {
    setIsHintUsed(team.egyptQuizz.question2.hintUsed);
  }, [team.egyptQuizz.question2.hintUsed]);

  const handleHintUsage = () => {
    setIsHintUsed(true);
    team.egyptQuizz.question2.hintUsed = true;
    team.points -= 50;
  ...
  const handlePassing = () => {
    team.egyptQuizz.question2.isCorrect = true;
    team.points += 250;
    localStorage.setItem('team', JSON.stringify(team));
  ...
  const handleSurrender = () => {
    team.egyptQuizz.question2.isCorrect = false;
    team.points -= 100;
    localStorage.setItem('team', JSON.stringify(team));
    dispatch(deductPoints(100));
    dispatch(setProfile(team));
    quizzRedirection('/egypt/3');
  };
    ...
    <div className="flex flex-col">
      ...
      >
        <h2 className="text-xl text-center bg-[#3B2F2F] p-2 rounded-md text-white font-bold">
          {[...activeLanguage.EGYPT_QUIZZ_QUESTION_2].reverse().join('')}
        </h2>
        <Input
          value={answer}
        ...
        <QuizzControls
          ...
          nextPage={'/quizz/egypt/3'}
          isHintUsed={isHintUsed}
          setIsHintUsed={handleHintUsage}
          onPassing={handlePassing}
          onSurrender={handleSurrender}
          activeHint={activeLanguage.EGYPT_QUIZZ_QUESTION_2_HINT}
        />
      </form>
    </div>
  );
